    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        return http
                ...
                .logout(httpSecurityLogoutConfigurer -> httpSecurityLogoutConfigurer
                        .logoutUrl("/user/logout")      // 로그아웃 수행 url 지정
                        //.logoutSuccessUrl("/login?logout")  // 로그아웃 성공후 redirect 하고자 하는 url
                        .invalidateHttpSession(false)   // session invalidate (디폴트 값은 true) 세션 이 날라가는 것
                        // 이따가 CustomLogoutSuccessHandler 에서 꺼낼 정보 가있기 때문
                        // false 로 세팅  (로그인 시간과 로그아웃 시간을 대조하여 사이트에 머무른 시간을 계산하기위함)
